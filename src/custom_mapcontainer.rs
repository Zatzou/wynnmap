...
use leptos_leaflet::prelude::JsWriteSignal;

use super::{MapEvents, PopupEvents, Position, TooltipEvents, provide_leaflet_context};
...
pub fn MapContainerSCRS(
    ...
    Effect::new(move |_| {
        if let Some(map_div) = map_load.get() {
            ...
            options.set_crs(Simple.with(JsValue::clone));
            // options.set_crs(JsValue::from_str("Simple"));
...
extern "C" {
    ...
    #[wasm_bindgen(js_namespace = ["L", "CRS"], js_name = "Simple", thread_local_v2)]
    pub static Simple: JsValue;
}
