...
There are two components as of right now, the backend server and the frontend. To get started, you will need to install [Rust](https://rustup.rs/) and [Trunk](
https://trunk-rs.github.io/trunk/guide/getting-started/installation.html). Npm is also required to use the tailwindcss cli. The tailwind cli can be installed using `npm i`.
